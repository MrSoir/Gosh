...
long StaticFunctions::evaluateContentCount(const QFileInfo& entry)
{
    if( !entry.exists() )
        return 0;
...
bool StaticFunctions::copyFolder(std::shared_ptr<bool> cancelled,
                                 ...
                                 std::function<void()> callAfterProgress)
{
//    qDebug() << "in copyFolder: sourcePath: " << absSourcePath << " tarPath: " << tarPath << "  tarName: " << tarName << "  deleteSource: " << deleteSource;
    if(cancelled && *cancelled)
        return false;
...
//      if(tarPath.startsWith(absSourcePath))
        {
//            showInfoDialog(QObject::tr("must not copy parent-folder into sub-folder!"), QObject::tr("invalid operation"));
            qDebug() << "must not copy parent-folder into subfolder:"
                     << "   parent: " << absSourcePath
                     << "   target: " << tarPath;
            return false;
        }
...
//        qDebug() << "       -> tarFolderPath: " << tarFolderPath << "   uniqueTarFolderName: " << uniqueTarFolderName;
//        return true;
        ...
        if(succ_created){
            QFileInfoList content = StaticFunctions::getEntriesInDirectory(QDir(absSourcePath), true);//QDir(absSourcePath).entryInfoList(QDir::AllEntries | QDir::Hidden);
...
//                  if( !absSourcePath.startsWith( cont.absoluteFilePath() ) )
                    {
                        if(cancelled && *cancelled)
                            return false;
...
void StaticFunctions::zipFile(std::shared_ptr<bool> cancelled,
